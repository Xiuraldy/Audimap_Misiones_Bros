import './CommonsMissionEight.css' 
...
import audioEight from '../../../assets/audio/missionEight.m4a'
import audiowin from '../../../assets/audio/winner.mp3' 

export const CommonsMissionEight = () => {
    ...
    const {getInfUser, nevel, setNotesEight, notesEight} = useGetInfUser()
    useEffect(() => {
        // console.log("id", id)
        if(id){
            getInfUser(id) 
    ...
    const {contentTextMission, writeObject} = useWriteObject("Suban el informe a Conexión.")
    ...
    const audio = new Audio(audioEight); //Audio del Objetivo
    ...
    const completedMission = () => {
        ...
        audioWinner.play()
        nevel < 8 && sendRequestEdit(id)
        sendRequestValues('missionEight', id)
        onModalCompleted()
        writeWinner()
    }

    const redirectLink = () => {
        window.open('https://drive.google.com/file/d/1QfszlnMRdVS4pWF_luajJY98wmx5oUUf/view?usp=sharing')
    }
    ...
    return (
        <>
            ...
            <button className='send-mission'>{nevel >= 8 ? 'Misión Completada' : 'Misión Pendiente'}</button>
            <div className="content-mission">
                <div className="tasks">
                    <div className="program-meet">
                        <div className="text-principal">
                            <h1 className='link-calendar' name="missionThree" onClick={() => redirectLink()}>Video: Paso a Paso ~ Cargue Masivo<img src={ imgcursor } className='cursor-gif' /></h1>
                            <div className="paste-link" name="missionEight">
                                <h3 className='description-missionThree'><i>1.</i> Descarga la plantilla que aparece en el informe en conexión
                                <br/><i>2.</i> Copia tu información de la lista de chequeo y pégala a la plantilla
                                <br/><i>3.</i> Verifica en que la plantilla no tenga espacios en blanco debajo de la última fila
                                <br/><i>4.</i> Guarda el documento como .txt (Texto delimitado por tabulaciones)
                                <br/><i>5.</i> Carga el archivo en el link que aparece en el informe en conexión
                                <br/><i>6.</i> ¡Listo!<br/><b>Peguen el link del informe en conexion para completar misión</b></h3>
                                <textarea
                                    type="text" 
                                    placeholder='Link'
                                    className='notes-missionThree' 
                                    onChange={(e) => setNotesEight(e.target.value)} 
                                    value={notesEight}
                                />
                            </div>
                        </div>
