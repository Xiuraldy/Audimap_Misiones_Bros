...
import audioFour from '../../../assets/audio/missionFour.m4a'
import audiowin from '../../../assets/audio/winner.mp3' 

export const CommonsMissionFour = () => {
    ...
    const {contentTextMission, writeObject} = useWriteObject("Elaboren la lista de chequeo en Drive.")
    ...
    const audio = new Audio(audioFour); //Audio del Objetivo
    ...
    const completedMission = () => {
        nevel < 4 && sendRequestEdit(id)
        onModalCompleted()
        writeWinner()
        audioWinner.play()
        window.open(links.areaAuditar)
    }
    ...
    return (
        <>
            ...
            <button className='send-mission'>{nevel >= 4 ? 'Misión Completada' : 'Misión Pendiente'}</button>
            <div className="content-mission">
                <div className="tasks" name="center">
                    <div className="program-meet">
                        <h1 className='link-calendar' onClick={() => completedMission()}>Busca en este link el área a auditar<img src={ imgcursor } className='cursor-gif' /></h1>
                    </div>
                </div>
            </div>
            {/* Modal Del Objetivo */}
